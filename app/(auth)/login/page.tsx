...
export default async function LoginPage({ searchParams }: Props) {
  ...
  try {
    ...
    // ── 1. Re-throw Next.js control-flow errors (redirect / notFound / etc.) ──
    //    redirect() works by throwing an error whose `digest` starts with
    //    "NEXT_REDIRECT". If we swallow it, the user is never redirected.
    const digest = (err as { digest?: string })?.digest;
    if (typeof digest === "string" && digest.startsWith("NEXT_")) {
      throw err;
    }

    // ── 2. Log the FULL error so the real cause is visible in the terminal ──
    console.error("[login] getSession() failed:", err);

    // ── 3. Only flag DB errors for the ones we can positively identify ──
    ...
    const isDbError =
      ...
      code === "ENOTFOUND" ||
      code === "ETIMEDOUT" ||
      code === "28P01" || // invalid_password (Postgres SQLSTATE)
      code === "3D000" || // invalid_catalog_name (db does not exist)
      code === "42P01" || // undefined_table (migrations not run)
      /ECONNREFUSED|AggregateError|password authentication failed|does not exist|Can't reach database server/i.test(
        msg,
      );
    ...
    if (isDbError) {
      ...
      // Not a DB problem — surface it instead of hiding it behind the panel.
      throw err;
    }
  }
    ...
    <div className="flex min-h-screen">
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between bg-indigo-600 p-12 relative overflow-hidden">
        ...
        <div className="relative space-y-4">
          ...
          <p className="text-indigo-200 text-base leading-relaxed max-w-xs">
            Official academic records platform for managing student transcripts,
            grades, and academic history.
          </p>
          <ul className="space-y-3 pt-4">
            {[
              "Secure role-based access",
              "Bulk student & grade uploads",
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 bg-white dark:bg-gray-950">
        ...
        <div className="w-full max-w-sm">
          {dbError ? (
            <DatabaseErrorPanel detail={dbErrorMessage} />
          ) : (
            <>
              <div className="mb-8">
...
function DatabaseErrorPanel({ detail }: { detail?: string }) {
  return (
    <div className="space-y-4">
      ...
      <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
        <li className="flex gap-2">
          <span className="text-red-500 font-bold">1.</span> Is PostgreSQL
          running? Open Windows Services and check{" "}
          <strong>postgresql-x64-16</strong>
        </li>
        <li className="flex gap-2">
          <span className="text-red-500 font-bold">2.</span> Is{" "}
        ...
        <li className="flex gap-2">
          <span className="text-red-500 font-bold">3.</span> Did you run{" "}
          <code className="bg-gray-100 dark:bg-gray-800 px-1 rounded">
            pnpm db:push
          </code>{" "}
      ...
      {detail ? (
        <pre className="mt-2 max-h-32 overflow-auto rounded-lg bg-gray-100 dark:bg-gray-900 p-3 text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap break-all">
          {detail}
        </pre>
      ) : null}
