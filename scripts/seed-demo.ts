...
async function insertIfAbsent<T extends { id: string }>(
  ...
  if (existing.length > 0) {
    console.log(
      `  skip  ${label} (already exists, id=${existing[0].id.slice(0, 8)})`,
    );
  ...
  const [row] = (await db
    ...
    .returning({ id: (table as any).id })) as { id: string }[];
...
async function main() {
  ...
  const regRows = await db
    .select({ id: registrar.id })
    .from(registrar)
    .where(eq(registrar.isActive, true))
    .limit(1);
  if (regRows.length === 0) {
  ...
  const COURSES = [
    ...
    { code: "ENG101", title: "Engineering Drawing", creditHours: 3 },
    {
      code: "GEN001",
  ...
  for (const s of SEMESTERS) {
    const existing = await db
      ...
      .from(semesters)
      .where(
        and(eq(semesters.year, s.year), eq(semesters.semester, s.semester)),
      )
      .limit(1);
    ...
    if (existing.length > 0) {
      ...
      const [row] = await db
        .insert(semesters)
        .values({ year: s.year, semester: s.semester })
  ...
  for (const s of STUDENTS) {
    const existing = await db
      ...
      .from(students)
      .where(eq(students.indexNumber, s.indexNumber))
      .limit(1);
    if (existing.length > 0) {
  ...
  for (const entry of GRADE_ENTRIES) {
    ...
    if (!studentId || !courseId || !semesterId) {
      console.log(
        `  warn  Missing FK for ${entry.studentIdx} / ${entry.courseCode} / ${entry.semKey}`,
      );
      continue;
    }
  ...
  console.log(
    `  ✓     ${gradeInserted} grade${gradeInserted !== 1 ? "s" : ""} inserted, ${gradeSkipped} skipped`,
  );
